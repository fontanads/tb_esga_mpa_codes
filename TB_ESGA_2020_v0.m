...
EbN0dB = 15; %10:2:22;                                                            % EbN0 [dB] points to simulate
Pe = 1e-3;                                                                  % acceptable probability of error
min_num_of_errors = 50;                                                      % desired number of frame errors per point
iter_cnt_saving_threshold = 5;                                              % saving temp mat files every #x iterations
print_frequency = 5;                                                        % frequency of iterations to print simulation status

%%
% SCMA parameters
load_factor = 1.67;                    % \beta: load factor of SCMA J/K
num_of_subcarriers = 12;               % K: number of shared resources
modulation_spectral_eficiency = 3;    % kb = log2(M): bits/symbol 
spreading_degree = 3;                 % N: number of complex dimensions of each symbol
alphabet_size = 3;                    % q: number of unique complex projections in each complex dimension
code_rate = 1/2;
...
r_th = 0.30;                              % r: threshold parameter for TB-ESGA-MPA (0 <= r <= 1), where 0 corresponds to original MPA and 1 corresponds to full Gaussian Approx.
...
%                      [a_priori_beliefs] = modulation_pmf_update(soft_bits_MPA_input,codewords_binary_labels);
            ...
            switch sys.OS_str
                case 'Linux'
%                     [soft_bits_MPA_output, hard_symbols] = TB_ESGA_MPA_v0_Linux(y,F,F_int,C,C_perms, Aq, codewords_binary_labels, H, Tm_first,r_th,a_priori_beliefs, 2*noise_var);
                    [soft_bits_MPA_output, hard_symbols] = TB_ESGA_MPA_v0_Linux_mex(y,F,F_int,C,C_perms, Aq, codewords_binary_labels, H, Tm,r_th,a_priori_beliefs, 2*noise_var);
                 case 'Windows'
%                      [soft_bits_MPA_output, hard_symbols] = TB_ESGA_MPA_v0(y,F,F_int,C,C_perms, Aq, codewords_binary_labels, H, Tm_first,r_th,a_priori_beliefs, 2*noise_var);
                     [soft_bits_MPA_output, hard_symbols] = TB_ESGA_MPA_v0_mex(y,F,F_int,C,C_perms, Aq, codewords_binary_labels, H, Tm,r_th,a_priori_beliefs, 2*noise_var);
                 otherwise
                    error('Operational System not supported.')
            end
