/*
 * _coder_TB_ESGA_MPA_v0_Linux_api.c
 *
...
#include "_coder_TB_ESGA_MPA_v0_Linux_api.h"
  ...
  emlrtMsgIdentifier *msgId, emxArray_real_T *ret)
{
  ...
  int32_T iv11[3];
  emlrtCheckVsBuiltInR2012b(sp, msgId, src, "double", false, 3U, dims, &bv7[0],
    iv11);
  ret->size[0] = iv11[0];
  ret->size[1] = iv11[1];
  ret->size[2] = iv11[2];
  ...
  emlrtMsgIdentifier *msgId, emxArray_creal_T *ret)
{
  ...
  ret->size[1] = iv4[1];
  ret->size[2] = iv4[2];
  emxEnsureCapacity_creal_T1(sp, ret, i12, (emlrtRTEInfo *)NULL);
...
void TB_ESGA_MPA_v0_Linux_api(const mxArray * const prhs[12], int32_T nlhs,
  const mxArray *plhs[2])
{
  ...
  TB_ESGA_MPA_v0_Linux(&st, y_all, F, F_int, C_data, C_size, *C_perms_data,
                       C_perms_size, Aq_data, Aq_size,
                       *codewords_binary_labels_data,
                       codewords_binary_labels_size, H, Tm, r_th,
                       a_priori_beliefs, varnoise, soft_bits, hard_symbols);
...
/* End of code generation (_coder_TB_ESGA_MPA_v0_Linux_api.c) */
