function [soft_bits, hard_symbols] = TB_ESGA_MPA_v0_Linux(y_all,F,F_int,C,C_perms, Aq, codewords_binary_labels, H, Tm,r_th,a_priori_beliefs, varnoise)
        ...
        for k=1:K % for every FN
            ...
            for j1=1:length(phi_k)
                ...
                for q_id = 1:q % reading all possible Projections of the codebook of user_j @ (resource k)
                    ...
                    for nr=1:Nr % for each receive antenna
                        ...
                        yk_fix = y(k,nr) - H(k,j,nr)*Aq(q_id) - adapt_mean;    % received sample - channel*fixed_projection_of_user_j - effective_noise_mean
                        ...
                        for marginal_prob_idx_cnt=1:prod(length_of_each_counter) % begin interference marginalization: 
                            ...
                            h_strong_users = squeeze(H(k,strong_users,nr));
                            interference = h_strong_users*interference_projections;
                        ...
                        end % fim da marginaliza��o sobre a interfer�ncia
                    end
...
%         h_kj = squeeze(H(k,j,nr));       % channel fading of user j in FN k @ Rx antenna nr
    ...
    function [mean_u, var_u] = get_mean_and_var(user) % get expecation of "user" projection E[X(k,u)] and E[|X(k,u)|^2]
        ...
        h_kj_nr = H(k,user,nr); % fading coefficient of user 
        for mm=1:M
